...
There are 32. bits of data, so the value wraps around every few hours."
  (declare (ignore INHIBIT-SCHEDULING-FLAG))
  (SELECT-PROCESSOR
    (:CADR
...
(DEFCONST INTERNAL-TIME-UNITS-PER-SECOND 1000.
  "60 60ths of a second in a second.")
...
(DEFUN GET-INTERNAL-RUN-TIME ()
  "Returns time in 60'ths since last boot. May be a bignum."
  #|(values (floor (* 60 (cl:get-internal-real-time)) 1000))|#
  (cl:get-internal-real-time))

(DEFF GET-INTERNAL-REAL-TIME 'cl:GET-INTERNAL-RUN-TIME)
...
(DEFVAR *CUMULATIVE-MONTH-DAYS-TABLE*
        (MAKE-ARRAY 13. :element-type '(unsigned-byte 16)
                        :INITIAL-CONTENTS '#(0   0   31  59  90  120 151
                                                 181 212 243 273 304 334))
  "One-based array of cumulative days per month." )
...
(defun first-sunday-in-april (year)
  (let* ((day-year-started-on (day-of-week-first-day-of-year year))
    ...
    (1+ (cl:rem (- 7 day-of-week-april-first) 7)))) ;? 1+
...
(defun tv..notify (stream format-string &rest args)
...
;                       (NULL (UNIX-PROCESSOR-PRESENT))  ;It sets up SDU clock differently, let it have it.
        ...
        (:cadr)
        (:x86-64
         (setq ut (cl:get-universal-time))
         (go do-it)))
   STRING
      (FORMAT *QUERY-IO* "~&Please type the date and time: ")
      (SETQ UT (READ-LINE *QUERY-IO*))
      ...
      (CONDITION-CASE (ERROR)
        ...
        (ERROR (print ">> error foo?" *QUERY-IO*)
               (GO STRING)))
   GIVE-IT-A-SHOT
...
#|(DEFUN PRINT-UPTIME (&OPTIONAL (STREAM *STANDARD-OUTPUT*))
  ...
  (FORMAT STREAM "~&This machine has been up ~\time-interval\."
          (- (GET-UNIVERSAL-TIME) *UT-AT-BOOT-TIME*)))|#

(DEFUN PRINT-UPTIME (&OPTIONAL (STREAM *STANDARD-OUTPUT*))
  "Print how long this machine has been up since last cold boot."
  (FORMAT STREAM "~&This machine has been up ~A."
...
;|(kl:defconstant* rtc-cookie "C'est vrai.")
;|
...
;|      (decode-universal-time-without-dst ut)
