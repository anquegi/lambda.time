...
;; (DEFVAR *TIME-PACKAGE* (FIND-PACKAGE "TIME") "The package TIME")
;; XXX
(DEFVAR *TIME-PACKAGE* (FIND-PACKAGE :lambda.time.internal) "The package TIME")
             ...
             (DO ((I INDEX (1+ I)))
                 ((OR (≥ I END)
                      (LET ((CHAR (AREF STRING I)))
                        (AND (NOT (OR (char= CHAR #\@) (ALPHA-CHAR-P CHAR)))
...
(DEFUN PATTERN-INVOKE (PATTERN TOKEN-LIST FUNCTION PASS-ARGUMENTS
  ...
  (PROG ()
     ...
     END-LOOP
        ...
        (apply function (reverse pdl-room))
        (RETURN TOKEN-LIST)))
...
;;; Absolute values.
(DEFVAR *ABS-YEAR* nil)
(DEFVAR *ABS-MONTH* nil)
(DEFVAR *ABS-DATE* nil)
(DEFVAR *ABS-HOUR* nil)
(DEFVAR *ABS-MINUTE* nil)
(DEFVAR *ABS-SECOND* nil)
(DEFVAR *ABS-DAY-OF-THE-WEEK* nil)
(DEFVAR *ABS-TIME-ZONE* nil)

;;; Relative values, from offsets.
(DEFVAR *REL-YEAR* nil)
(DEFVAR *REL-MONTH* nil)
(DEFVAR *REL-DATE* nil)
(DEFVAR *REL-HOUR* nil)
(DEFVAR *REL-MINUTE* nil)
(DEFVAR *REL-SECOND* nil)
(DEFVAR *REL-DAY-OF-THE-WEEK* nil)
;(DEFVAR *REL-TIME-ZONE*)

;;; Values of the "base" time.
(DEFVAR *BASE-YEAR* nil)
(DEFVAR *BASE-MONTH* nil)
(DEFVAR *BASE-DATE* nil)
(DEFVAR *BASE-HOUR* nil)
(DEFVAR *BASE-MINUTE* nil)
(DEFVAR *BASE-SECOND* nil)

(DEFVAR *RELATIVE-P* nil)

;;; Action functions.
...
(DEFUN PARSE (STRING &OPTIONAL (START 0) END (FUTUREP T)
  ...
  ;; (DECLARE (RETURN-LIST SECS MINUTES HOURS DAY MONTH YEAR DAY-OF-THE-WEEK DAYLIGHT-SAVINGS-P RELATIVE-P))
...
DAY-MUST-BE-VALID if NIL means allow things like February 29
 (which equals March 1 or March 2)."
  ...
  (block KLUDGE
   (PROG ()                               ;This is needed because multiple values
...
return the universal time for it.  Otherwise, return NIL."
  (PROG (IDX YEAR MONTH DATE HOUR MINUTE SECOND SUBSTRING)
        (IF (AND (> END START) (char= (AREF STRING START) #\SPACE))
            (INCF START))
...
(DEFPROP :TIME-INTERVAL-OR-NEVER (PRINT-INTERVAL-OR-NEVER READ-INTERVAL-OR-NEVER
  ...
  'TV>CHOOSE-VARIABLE-VALUES-KEYWORD)   ;todo
